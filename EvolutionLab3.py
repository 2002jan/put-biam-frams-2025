...
from FramsticksEvolution import ensureDir, save_genotypes, is_feasible_fitness_criteria, \
    frams_crossover, frams_mutate, selTournament_only_feasible, selNSGA2_only_feasible, frams_getsimplest, \
    genotype_within_constraint, FITNESS_VALUE_INFEASIBLE_SOLUTION
...
def frams_evaluate(frams_lib, individual):
    ...
    if not hasattr(frams_evaluate, "eval_counter"):
        print("Creating eval_counter for frams_evaluate")
        frams_evaluate.eval_counter = 0

    frams_evaluate.eval_counter += 1

    if parsed_args.dynamic_params:
        import frams
        # fill this section
        TRANSITION_LENGTH = 150 * 50 # 150 generations, 50 evaluations per generation
        progress = min(1.0, frams_evaluate.eval_counter / TRANSITION_LENGTH)
        new_probs = {
            "f1_smX":        (1 - progress) * 3.0 + progress * 1.0,
            "f1_smJunct":    (1 - progress) * 2.5 + progress * 0.5,
            "f1_smComma":    (1 - progress) * 2.0 + progress * 0.75,
            "f1_smModif":    (1 - progress) * 5.0 + progress * 2.0,
            "f1_nmNeu":      (1 - progress) * 3.0 + progress * 1.5,
            "f1_nmConn":     (1 - progress) * 3.0 + progress * 1.5,
            "f1_nmProp":     (1 - progress) * 1.0 + progress * 2.5,
            "f1_nmWei":      (1 - progress) * 1.0 + progress * 2.5,
            "f1_nmVal":      (1 - progress) * 0.5 + progress * 2.0,
        }
        # print 5 times the new probabilities depending on eval_counter
        if frams_evaluate.eval_counter % 250 == 0:  # print every 250 evaluations
            import pprint
            print("Progress:", progress)
            print("New mutation probabilities:")
            pprint.pprint(new_probs)
            print("\n")

        for param_name, value in new_probs.items():
            if hasattr(frams.GenMan, param_name):
                setattr(frams.GenMan, param_name, value)
            else:
                print(f"Warning: GenMan has no attribute named '{param_name}'")
...
def parseArguments():
    ...
    parser.add_argument("-dynamic-params", type=bool, default=False, action=argparse.BooleanOptionalAction)
...
def main():
    ...
    if parsed_args.hof_savefile is not None and False:
        save_genotypes(parsed_args.hof_savefile, OPTIMIZATION_CRITERIA, hof)

    os.makedirs("outputs_lab3", exist_ok=True)

    genes, maxes, stdes = log.select("gen", "max", "stddev")

    file_name = f"outputs_lab3/out_{parsed_args.run_type}_{parsed_args.run_number}.csv"
    ...
    data = {
        ...
        'max': maxes,
        'stdev': stdes,
        'runtime': [run_time for _ in range(len(genes))]
    }

    df = pd.DataFrame(data, columns=['gen', 'max', 'stdev', 'runtime'])
