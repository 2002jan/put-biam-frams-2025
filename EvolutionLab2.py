...
def frams_evaluate_new(frams_lib, individual):
    ...
    if fitness[0] < 0:
        fitness[0] = max(default_evaluation_data['numparts'] * default_evaluation_data['numjoints'], 1) ** -1
        fitness[0] *= -1
...
def prepareToolbox(frams_lib, OPTIMIZATION_CRITERIA, tournament_size, genetic_format, initial_genotype, new_fitness_fn=False):
    ...
    toolbox.register("evaluate", frams_evaluate_new if new_fitness_fn else frams_evaluate, frams_lib)
...
def parseArguments():
    ...
    parser.add_argument("-new-fitness", type=bool, default=False, action=argparse.BooleanOptionalAction)
...
def main():
    ...
    toolbox = prepareToolbox(framsLib, OPTIMIZATION_CRITERIA, parsed_args.tournament, '1' if parsed_args.genformat is None else parsed_args.genformat, parsed_args.initialgenotype, parsed_args.new_fitness)
    ...
    mutation_rate = str(frams.GenMan.f9_mut).replace(".", "")

    os.makedirs("outputs_lab2", exist_ok=True)

    genes, maxes, stdes = log.select("gen", "max", "stddev")

    fitness_type = "nf" if parsed_args.new_fitness else "df"
    file_name = f"outputs_lab2/out_{fitness_type}_{parsed_args.genformat}_{parsed_args.run_number}.csv"
