...
def main():
    ...
    file_index = random.randint(100000, 9999999)
    file_name = lambda x: f"outputs/out_{mutation_rate}_{x}.csv"

    while os.path.exists(file_name(file_index)):
        file_index += 1
    ...
    df = pd.DataFrame(data, columns=['gen', 'max', 'stdev'])
    # df.to_csv(file_name(file_index))
